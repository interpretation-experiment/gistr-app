...
export default IntroJSComponent.extend(SessionMixin, {
  ...
  initIntroJS: function() {
    if (this.get('lifecycle.isInExp')) {
      this.set('steps', this.get('getExpSteps')(this.get('currentUser'), this.get('shaping')));
    } else if (this.get('lifecycle.isInPlaying')) {
      this.set('steps', this.get('getPlayingSteps')(this.get('currentUser'), this.get('shaping')));
    } else {
